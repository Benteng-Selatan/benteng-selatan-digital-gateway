...
type Detail = {
  id: string; requestNumber: string; status: ServiceRequestStatus; applicantName: string; identityNumber: string; familyCardNumber: string; phone: string; address: string; formData: Record<string,string>; citizenNote: string; publicNote: string; submittedAt: string; updatedAt: string;
  messages: { id: string; senderType: string; senderLabel: string; message: string; createdAt: string }[];
  history: { id: string; newStatus: string; publicNote: string; createdAt: string }[];
};
  ...
  return <div className="portal-grid">
    <section className="portal-main-column">
      <section className="portal-panel"><div className="portal-panel-heading"><div><span className={`workflow-badge ${request.status}`}>{REQUEST_STATUS_LABELS[request.status]}</span><h2>{request.requestNumber}</h2><p>Surat Keterangan Usaha</p></div></div>
        ...
        {request.publicNote ? <div className="notice warning"><div><strong>Catatan petugas</strong><p>{request.publicNote}</p></div></div> : null}
      </section>
